...
public class CurrencyConversionMenu extends AbstractContainerMenu
{
    private final ContainerLevelAccess access;
    private final DataSlot selectedRecipeIndex = DataSlot.standalone();
    private final Level level;
    private List<CurrencyConversionRecipe> recipes = new ArrayList<>();
    ...
    public CurrencyConversionMenu(int id, Inventory playerInventory) {
        this(id, playerInventory, ContainerLevelAccess.NULL);
    }

    public CurrencyConversionMenu(int id, Inventory playerInventory, final ContainerLevelAccess access) {
        super(MenuTypeInit.CURRENCY_CONVERSION.get(), id);
        this.access = access;
    ...
    public List<CurrencyConversionRecipe> getRecipes() {
        return this.recipes;
    }
    ...
    public boolean stillValid(Player player) {
        return stillValid(this.access, player, BlockInit.CURRENCY_CONVERSION_TABLE.get());
    }
    ...
    private void setupRecipeList(Container inventoryIn, ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            this.recipes = RecipeHelper.getSortedMatchingRecipesCurrencyConversion(this.level, inventoryIn);
        }
    }

    private void setupResultSlot() {
        if (!this.recipes.isEmpty() && isValidRecipeIndex(this.selectedRecipeIndex.get())) {
            CurrencyConversionRecipe recipe = this.recipes.get(this.selectedRecipeIndex.get());
    ...
    public MenuType<?> getType() {
        return MenuTypeInit.CURRENCY_CONVERSION.get();
    }
    ...
    public ItemStack quickMoveStack(Player playerIn, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            } else if (this.level.getRecipeManager().getRecipeFor(RecipeTypeInit.CURRENCY_CONVERSION, new SimpleContainer(itemstack1), this.level).isPresent()) {
